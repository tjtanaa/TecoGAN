...
def gen_frames(infile, outdir, width, height, start, duration, savePNG=True):
    ...
    if savePNG:
        ...
        for i in range(duration):
            ...
            filename = (outdir+'col_high'+("_%04d.png"%(i))) if index == -1 else (outdir+'col_high'+("_compressed_%04d.png"%(i)))
    ...
    for key in video_data_dict.keys():
        ...
        for track in media_info.tracks:
            if track.track_type == 'Video':
                print("Resolution: {} x {}, Bit Rate:{}".format(track.width, track.height, track.bit_rate))

        aspect_ratio = '{}x{}'.format(media_info.tracks[1].width//4, media_info.tracks[1].height//4)
        video_bitrate = '50k'
        video_codec = 'libx264'
        audio_codec = 'copy'
...
def prepare_frames():
    ...
    for key in video_data_dict.keys():
        ...
        if (not os.path.isfile(lr_input_video_path)):
            ...
            continue
        hr_media_info = MediaInfo.parse(hr_input_video_path)
        lr_media_into = MediaInfo.parse(lr_input_video_path)
        # get training frames
        for start_fr in video_data_dict[key]:
            ...
            gen_frames(hr_input_video_path, tar_dir, hr_media_info.tracks[1].width, hr_media_info.tracks[1].height, start_fr, Flags.duration)
            gen_frames(lr_input_video_path, tar_dir, lr_media_into.tracks[1].width, lr_media_into.tracks[1].height, start_fr, Flags.duration)
            cur_id = cur_id+1
